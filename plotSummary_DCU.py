...
def getTGraph(modPart,layer,color):
	...
	for t in range(len(dtime_t_2)):
		...
		for mod in part.keys():
			if modPart not in part[mod]: continue
			if pos[mod]!=layer: continue
			if dcuileak[mod][t]>0 and abs(dcuileak[mod][t] - ileakc_t_on[mod][t])/dcuileak[mod][t]<0.6 and abs(dcutemp[mod][t]+273.16 - temp_t_on[mod][t])/(dcutemp[mod][t]+273.16)<0.6:
...
outRfile = TFile(outDir+"/summaries"+postFix+".root",'RECREATE')
